...
const REDIS_KEY = 'dmrv:grid-status:v1';
let cached = null;
let cachedAt = 0;
const CACHE_TTL = 180_000; // 3min — grid data is near-real-time
...
export default async function handler(req) {
  ...
  if (!cached) {
    return jsonResponse({ error: 'Grid status data unavailable' }, 503, {
      'Cache-Control': 'no-cache', ...corsHeaders,
    });
  }

  return jsonResponse(cached, 200, {
    'Cache-Control': 's-maxage=300, stale-while-revalidate=120, stale-if-error=900',
    ...corsHeaders,
  });
}
