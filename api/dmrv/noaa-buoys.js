...
const REDIS_KEY = 'dmrv:noaa-buoys:v1';
let cached = null;
let cachedAt = 0;
const CACHE_TTL = 600_000; // 10min
...
export default async function handler(req) {
  ...
  if (!cached) {
    return jsonResponse({ error: 'NOAA buoy data unavailable' }, 503, {
      'Cache-Control': 'no-cache', ...corsHeaders,
    });
  }

  return jsonResponse(cached, 200, {
    'Cache-Control': 's-maxage=3600, stale-while-revalidate=1800, stale-if-error=7200',
    ...corsHeaders,
  });
}
