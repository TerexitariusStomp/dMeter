...
export class DataLoaderManager implements AppModule {
  ...
  async loadMarkets(): Promise<void> {
    try {
      ...
      const fgSectorPerf = Array.isArray(hydratedFg?.sectorPerformance)
      ...
      if (hydratedSectors?.sectors?.length) {
        ...
        heatmapPanel?.renderHeatmap(hydratedSectors.sectors.map(toHeatmapItem), fgSectorPerf);
      } else {
        const sectorsResp = await fetchSectors();
        if (sectorsResp.sectors.length > 0) {
          heatmapPanel?.renderHeatmap(sectorsResp.sectors.map(toHeatmapItem), fgSectorPerf);
        } else if (stocksResult.skipped) {
