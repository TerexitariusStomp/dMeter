...
if [ -f "$OVERRIDE" ]; then
  ...
  grep -E '^\s+[A-Z_]+:' "$OVERRIDE" \
    ...
    | sed 's/: */=/' \
    | sed "s/[\"']//g" \
    | grep -E '^(NASA_FIRMS|FIRE_MAPS|GROQ|AISSTREAM|FRED|FINNHUB|ALPHA_VANTAGE|EIA|ACLED_ACCESS_TOKEN|ACLED_EMAIL|ACLED_PASSWORD|CLOUDFLARE|AVIATIONSTACK|OPENAQ_API_KEY|WAQI_API_KEY|AQICN_API_KEY|OPENROUTER_API_KEY|LLM_API_URL|LLM_API_KEY|LLM_MODEL|OLLAMA_API_URL|OLLAMA_MODEL|GIE_API_KEY|AGSI_API_KEY|ENTSO_E_TOKEN|ENTSOE_API_KEY|WTO_API_KEY|ABUSEIPDB_API_KEY|OTX_API_KEY|DATAVERSE_API_KEY|COMTRADE_API_KEYS|WINDY_)' \
