 ...
 * These are the exact JSON shapes that WorldMonitor's seed job and handlers consume.
 */
...
export interface WMOverviewSnapshot {
  ...
  overview: {
    ...
    retailerSpreadPct: number | null;
    coveragePct: number;
    freshnessLagMin: number | null;
  };
  topCategories: Array<{
    slug: string;
    name: string;
...
export interface WMMoversSnapshot {
  ...
  asOf: number;
  range: string;
  risers: Array<{ productId: string; title: string; category: string; retailerSlug: string; changePct: number }>;
  fallers: Array<{ productId: string; title: string; category: string; retailerSlug: string; changePct: number }>;
}
...
export interface WMFreshnessSnapshot {
  ...
  overallFreshnessMin: number | null;
  stalledCount: number;
}
...
export async function buildOverviewSnapshot(marketCode: string): Promise<WMOverviewSnapshot> {
  ...
  const indexResult = await query<{ metric_key: string; metric_value: string; metric_date: Date }>(
    `SELECT ci.metric_key, ci.metric_value, ci.metric_date
  ...
  for (const row of indexResult.rows) {
    metrics[row.metric_key] = parseFloat(row.metric_value);
  }

  const prevWeekResult = await query<{ metric_key: string; metric_value: string }>(
  ...
  for (const row of prevWeekResult.rows) {
    prevWeek[row.metric_key] = parseFloat(row.metric_value);
  }

  const ess = metrics.essentials_index ?? 100;
  const val = metrics.value_index ?? 100;
  const prevEss = prevWeek.essentials_index;
  const wowPct = prevEss ? ((ess - prevEss) / prevEss) * 100 : null;

  const spreadResult = await query<{ spread_pct: string }>(
  ...
  const categoryResult = await query<{
    ...
    `SELECT ci.category, ci.metric_key, ci.metric_value
     FROM computed_indices ci
     JOIN baskets b ON b.id = ci.basket_id
     WHERE b.market_code = $1
       AND ci.category IS NOT NULL
       AND ci.retailer_id IS NULL
       AND ci.metric_date = CURRENT_DATE
     ORDER BY ABS(ci.metric_value) DESC LIMIT 20`,
    [marketCode],
  );

  const currencyResult = await query<{ currency_code: string }>(
    `SELECT currency_code FROM retailers WHERE market_code = $1 AND active = true LIMIT 1`,
    [marketCode],
  );

  const freshnessResult = await query<{ avg_lag_min: string }>(
  ...
  return {
    ...
    overview: {
      ...
      weekOverWeekPct: wowPct ? Math.round(wowPct * 10) / 10 : null,
      monthOverMonthPct: null,
      retailerSpreadPct: spreadResult.rows[0]?.spread_pct ? parseFloat(spreadResult.rows[0].spread_pct) : null,
      coveragePct: metrics.coverage_pct ?? 0,
      freshnessLagMin: freshnessResult.rows[0]?.avg_lag_min ? parseInt(freshnessResult.rows[0].avg_lag_min, 10) : null,
    },
    topCategories: [],
  };
}
...
export async function buildMoversSnapshot(
  ...
  const result = await query<{
    ...
    `WITH latest AS (
       SELECT DISTINCT ON (rp.id) rp.id, rp.raw_title, rp.category_text, r.slug AS retailer_slug, po.price
...
export async function buildRetailerSpreadSnapshot(
  ...
  const retailers = result.rows.map((r) => ({
    ...
    freshnessMin: r.freshness_min ? parseInt(r.freshness_min, 10) : null,
  }));

  if (retailers.length > 0) {
    const cheapest = retailers[0].basketTotal;
    for (const r of retailers) {
      r.deltaVsCheapest = r.basketTotal - cheapest;
      r.deltaVsCheapestPct = cheapest > 0 ? ((r.basketTotal - cheapest) / cheapest) * 100 : 0;
    }
  }

  const spreadPct =
    retailers.length >= 2
      ? ((retailers[retailers.length - 1].basketTotal - retailers[0].basketTotal) / retailers[0].basketTotal) * 100
      : 0;
...
export async function buildFreshnessSnapshot(marketCode: string): Promise<WMFreshnessSnapshot> {
  ...
  const retailers = result.rows.map((r) => ({
    slug: r.slug,
    name: r.name,
    lastRunAt: r.last_run_at ? r.last_run_at.getTime() : null,
    status: r.last_run_status ?? 'unknown',
    parseSuccessRate: r.parse_success_rate ? parseFloat(r.parse_success_rate) : null,
    freshnessMin: r.freshness_min ? parseInt(r.freshness_min, 10) : null,
  }));

  const freshnessValues = retailers.map((r) => r.freshnessMin).filter((v): v is number => v !== null);
  const overallFreshnessMin =
    freshnessValues.length > 0
      ? Math.round(freshnessValues.reduce((a, b) => a + b, 0) / freshnessValues.length)
      : null;

  const stalledCount = retailers.filter((r) => r.freshnessMin === null || r.freshnessMin > 240).length;
